    ...
    def gradient(self, error, i, j):
        ...
        while timestamp < self.timestamp:
            ...
            while timestamp < max_timestamp:
                ...
                self.I.Weight[i][j] += error * self.dW1(activation_error, x, self.H.Weight[j][j], self.dh_dW1[i][j],
                                                        i, j)
                backprop_errors.append(self.dNext(error, activation_error, self.H.Weight[j][j], self.dh_dW1[i][j],
                                                        i, j))
...
if __name__ == '__main__':
    ...
    for _ in range(t):
        ...
        for _ in range(20):
            ...
            for j in range(o):
                for k in range(i):
                    R.gradient(error_vect[j]*tr, k, j)
            for vect in vects:
                R.feed_forward(vect)
        # print(R.get_output())
