...
class LSTMCell(AbstractRNN):
    ...
    def __init__(self, input_size, hidden_size, gate_activation="tanh", hidden_activation='tanh',
        ...
        super().__init__(input_size, hidden_size, gate_activation, hidden_activation, weight_param,
                         bias_params, bias_bool, fp)

        self.Vectors['s0'] = np.zeros(hidden_size)

        self.IG = Dense(input_size, hidden_size, None, weight_param, bias_params, bias_bool, False)
        self.IH = Dense(hidden_size, hidden_size, None, weight_param, bias_params, False, False)

        self.FG = Dense(input_size, hidden_size, None, weight_param, bias_params, bias_bool, False)
        self.FH = Dense(hidden_size, hidden_size, None, weight_param, bias_params, False, False)

        self.GG = Dense(input_size, hidden_size, None, weight_param, bias_params, bias_bool, False)
        self.GH = Dense(hidden_size, hidden_size, None, weight_param, bias_params, False, False)

        self.OG = Dense(input_size, hidden_size, None, weight_param, bias_params, bias_bool, False)
    ...
    def dWi(self, error, dh, dc, i, j, timestamp, training_rate):
        comp = (self.Vectors['x'+str(timestamp)][i] + self.IH.Weight[j][j]*self.dhp_dWi[i][j])
        dc *= comp
        dh *= comp
        self.dc_dWi[i][j] = dc
        self.dhp_dWi[i][j] = dh
        self.IG.Weight[i][j] += training_rate*error*dh

    def dUi(self, error, dh, dc, j1, j2, timestamp, training_rate):
        comp = (self.Vectors['h'+str(timestamp)][j1] + self.IH.Weight[j1][j2] * self.dhp_dUi[j1][j2])
        dc *= comp
        dh *= comp
        self.dc_dUi[j1][j2] = dc
        self.dhp_dUi[j1][j2] = dh
        self.IH.Weight[j1][j2] += training_rate * error * dh

    def dBi(self, error, dh, dc, j, training_rate):
        comp = (1 + self.IH.Weight[j][j]*self.dhp_dBi[j])
        dc *= comp
        dh *= comp
        self.dc_dBi[j] = dc
        self.dhp_dBi[j] = dh
        self.IG.Bias[j] += training_rate * error * dh
    ...
    def dWf(self, error, dh, dc, i, j, timestamp, training_rate):
        comp = self.Vectors['x'+str(timestamp)][i] + self.FH.Weight[j][j]*self.dhp_dWf[i][j]
        dc *= comp
        dh *= comp
        self.dc_dWf[i][j] = dc
        self.dhp_dWf[i][j] = dh
        self.FG.Weight[i][j] += training_rate*error*dh

    def dUf(self, error, dh, dc, j1, j2, timestamp, training_rate):
        comp = (self.Vectors['h' + str(timestamp)][j1] + self.FH.Weight[j1][j2] * self.dhp_dUf[j1][j2])
        dc *= comp
        dh *= comp
        self.dc_dUf[j1][j2] = dc
        self.dhp_dUf[j1][j2] = dh
        self.FH.Weight[j1][j2] += training_rate * error * dh

    def dBf(self, error, dh, dc, j, training_rate):
        comp = (1 + self.FH.Weight[j][j]*self.dhp_dBf[j])
        dc *= comp
        dh *= comp
        self.dc_dBf[j] = dc
        self.dhp_dBf[j] = dh
        self.FG.Bias[j] += training_rate * error * dh
    ...
    def dWg(self, error, dh, dc, i, j, timestamp, training_rate):
        comp = (self.Vectors['x'+str(timestamp)][i] + self.GH.Weight[j][j] * self.dhp_dWg[i][j])
        dc *= comp
        dh *= comp
        self.dc_dWg[i][j] = dc
        self.dhp_dWg[i][j] = dh
        self.GG.Weight[i][j] += training_rate*error*dh

    def dUg(self, error, dh, dc, j1, j2, timestamp, training_rate):
        comp = (self.Vectors['h' + str(timestamp)][j1] + self.GH.Weight[j1][j2] * self.dhp_dUg[j1][j2])
        dc *= comp
        dh *= comp
        self.dc_dUg[j1][j2] = dc
        self.dhp_dUg[j1][j2] = dh
        self.GH.Weight[j1][j2] += training_rate * error * dh

    def dBg(self, error, dh, dc, j, training_rate):
        comp = (1 + self.GH.Weight[j][j] * self.dhp_dBg[j])
        dc *= comp
        dh *= comp
        self.dc_dBg[j] = dc
        self.dhp_dBg[j] = dh
        self.GG.Bias[j] += training_rate * error * dh

    def dO(self, j, timestamp):
        dh = np.tanh(self.Vectors['s' + str(timestamp)])[j] * (self.A.errors["o" + str(timestamp)][j])
        return dh

    def dWo(self, error, dh, i, j, timestamp, training_rate):
        dh *= (self.Vectors['x'+str(timestamp)][i] + self.OH.Weight[j][j] * self.dhp_dWo[i][j])
        self.dhp_dWo[i][j] = dh
        self.OG.Weight[i][j] += training_rate * error * dh

    def dUo(self, error, dh, j1, j2, timestamp, training_rate):
        dh *= (self.Vectors['h' + str(timestamp)][j1] + self.OH.Weight[j1][j2] * self.dhp_dUo[j1][j2])
        self.dhp_dUo[j1][j2] = dh
        self.OH.Weight[j1][j2] += training_rate * error * dh

    def dBo(self, error, dh, j, training_rate):
        dh *= (1 + self.OH.Weight[j][j] * self.dhp_dBo[j])
        self.dhp_dBo[j] = dh
        self.OG.Bias[j] += training_rate * error * dh

    def gradient_h(self, error_vect, timestamp, training_rate=0.5):
...
def test():
    ...
    for _ in range(100):
        i, o = 20, 10
        R = LSTMCell(i, o, weight_param=(0, 1), bias_params=(0, 1), hidden_activation='softmax')
...
# if __name__ == "__main__":
    ...
    # for i in range(20):
    #     error_vect = np.subtract(y, R.get_output())
    #     R.gradient(error_vect, training_rate=0.3)
    #
    #     for vect in vects:
    #         R.feed_forward(vect)
    #
